...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    pqrCmd.x = kpBank * (CONSTRAIN((accelCmd.x / c), -sin(maxTiltAngle), sin(maxTiltAngle)) - R(0,2));
    pqrCmd.y = kpBank * (CONSTRAIN((accelCmd.y / c), -sin(maxTiltAngle), sin(maxTiltAngle)) - R(1,2));
    
    pqrCmd.x = (R(1,0) * pqrCmd.x - R(0,0) * pqrCmd.y)/R(2,2);
    pqrCmd.y = (R(1,1) * pqrCmd.x - R(0,1) * pqrCmd.x)/R(2,2);
...
float QuadControl::AltitudeControl(float posZCmd, float velZCmd, float posZ, float velZ, Quaternion<float> attitude, float accelZCmd, float dt)
{
  ...
  float thrust = 0;
     ...
     */
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    thrust = kpPosZ*(posZCmd - posZ) + kpVelZ*(velZCmd - velZ) + accelZCmd;
...
float QuadControl::YawControl(float yawCmd, float yaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    yawRateCmd = kpYaw * ( fmodf(yawCmd, 2*F_PI) - fmodf(yaw, 2*F_PI) );
  /////////////////////////////// END STUDENT CODE ////////////////////////////
