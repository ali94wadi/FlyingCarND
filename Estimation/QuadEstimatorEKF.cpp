...
//    ekfState(6) = ekfState(6) + dtIMU * gyro.z;	// yaw
    ...
    Quaternion<float> dq = q.IntegrateBodyRate(gyro, dtIMU);
    //Quaternion<float>::FromEuler123_RPY(gyro.x, gyro.y, gyro.z);
    //q.IntegrateBodyRate(gyro, dtIMU);
    Quaternion<float> attitude = dq;
...
//    VectorXf acc;
//    acc = Eigen::Map<Eigen::VectorXd>(accel.~V3F()    data(),accel.size()) ;
//    acc(1) = accel.x;acc(1) = accel.y;acc(2) = accel.z;
    predictedState.segment(0, 3) = curState.segment(0, 3) + dt * curState.segment(3, 3);
//    predictedState.segment(3, 3) = curState.segment(3, 3) + dt * accel;
    predictedState(3) = curState(3) + dt * accel.x;
    predictedState(4) = curState(4) + dt * accel.y;
    predictedState(5) = curState(5) + dt * accel.z - dt * CONST_GRAVITY;
//    predictedState(5) = predictedState(5) - CONST_GRAVITY * dt;
    /////////////////////////////// END STUDENT CODE ////////////////////////////
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
    ...
    gPrime(1,4) = dt;
    gPrime(2,5) = dt;
    gPrime(3, 6) = dt * accel.dot(RbgPrime(0));
    gPrime(4, 6) = dt * accel.dot(RbgPrime(1));
    gPrime(5, 6) = dt * accel.dot(RbgPrime(2));
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
    ...
    zFromX(4) = ekfState(4);
    zFromX(5) = ekfState(5);
//    hPrime.block(0, 0, 6, 6).Identity(6, 6);
    /////////////////////////////// END STUDENT CODE ////////////////////////////
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
    ...
    hPrime(0, 6) = 1;
    zFromX(0) = ekfState(6);
    float delta = ekfState(6) - magYaw;
    if(fabs(delta)>F_PI){
        zFromX(0) += (delta>0) ? -2.f*F_PI : 2.f*F_PI;
    }
    /////////////////////////////// END STUDENT CODE ////////////////////////////
